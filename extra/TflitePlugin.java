...
public class TflitePlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  private class RunSSDMobileNet extends TfliteTask {
    ...
    RunSSDMobileNet(HashMap args, ByteBuffer imgData, int numResultsPerClass, float threshold, Result result) {
      super(args, result);
      this.num = 5; // Adjusted to match the number of detected objects
      this.numResultsPerClass = numResultsPerClass;
      this.threshold = threshold;
      this.outputLocations = new float[1][num][8400]; // Adjusted for [1, 5, 8400] output shape
      this.outputClasses = new float[1][num]; // Adjusted for [1, 5] output shape
      this.outputScores = new float[1][num]; // Adjusted for [1, 5] output shape
    ...
    protected void onRunTfliteDone() {
      ...
      // Iterate through numDetections[0] (assuming it provides the number of valid
      // detections)
      for (int i = 0; i < numDetections[0]; ++i) {
        ...
        // Extract coordinates from outputLocations based on the model's output format
        float[] boxData = new float[4];
        System.arraycopy(outputLocations[0][i], 0, boxData, 0, 4);
        float ymin = boxData[0];
        float xmin = boxData[1];
        float ymax = boxData[2];
        float xmax = boxData[3];

        // Create rectangle map for each detection
        ...
        // Create result map for each detection
...
private class RunYOLO extends TfliteTask {
    ...
    RunYOLO(HashMap args,
        ...
        this.output = new float[1][5][8400];
    }
    ...
    // Inner class to represent a BoundingBox
    class BoundingBox {
    ...
    private List<BoundingBox> parseOutput(float[][][] output, float confidenceThreshold) {
        ...
        for (int i = 0; i < numBoxes; i++) {
            ...
            if (confidence > confidenceThreshold) {
                boxes.add(new BoundingBox(x, y, width, height, confidence));
            }
        }
    ...
    private float computeIoU(BoundingBox box1, BoundingBox box2) {
        float x1 = Math.max(box1.x - (box1.width/2) , box2.x - (box2.width/2));
        float y1 = Math.max(box1.y  - (box1.height/2), box2.y  - (box1.height/2));
        float x2 = Math.min(box1.x + (box1.width / 2), box2.x + (box2.width / 2));
        float y2 = Math.min(box1.y + (box1.height / 2), box2.y + (box2.height / 2));
    ...
    private List<BoundingBox> nonMaxSuppression(List<BoundingBox> boxes, float iouThreshold) {
        List<BoundingBox> result = new ArrayList<>();

        // Sort boxes by confidence score in descending order
        boxes.sort((b1, b2) -> Float.compare(b2.confidence, b1.confidence));

        while (!boxes.isEmpty()) {
            BoundingBox chosenBox = boxes.remove(0);
            result.add(chosenBox);

            boxes.removeIf(box -> computeIoU(chosenBox, box) > iouThreshold);
        }
    ...
    protected void onRunTfliteDone() {
        ...
        List<BoundingBox> boxes = parseOutput(output, threshold);
        List<BoundingBox> finalBoxes = nonMaxSuppression(boxes, 0.2f);
        ...
        for (BoundingBox box : finalBoxes) {
            //float xCenter = box.x * blockSize;
            //float yCenter = box.y * blockSize;
            //float width = box.width;
            //float height = box.height;

            //float xmin = Math.max(0, (xCenter - width / 2) / inputSize);
            //float ymin = Math.max(0, (yCenter - height / 2) / inputSize);
            //float xmax = Math.min(1, (xCenter + width / 2) / inputSize);
            //float ymax = Math.min(1, (yCenter + height / 2) / inputSize);

            Map<String, Object> rect = new HashMap<>();
            rect.put("x", box.x);
            rect.put("y", box.y);
            rect.put("w", box.width);
            rect.put("h", box.height);

            Map<String, Object> ret = new HashMap<>();
            ret.put("rect", rect);
            ret.put("confidenceInClass", box.confidence);
            ret.put("detectedClass", labels.get(0)); // Placeholder for detected class
